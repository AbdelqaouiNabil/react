...
async function upsertProfile(userId: string, role: 'admin' | 'student') {
  const { error } = await admin.from('profiles').upsert({ user_id: userId, role })
  if (error) throw error
}

async function upsertStudent(userId: string, firstName: string, lastName: string, email: string) {
  const { error } = await admin.from('ausbildung_main_engine').upsert({ user_id: userId, first_name: firstName, last_name: lastName, email })
  if (error) throw error
}

async function insertBewerbung(userId: string, title: string, company: string, status: string, agent: string) {
  const { error } = await admin.from('bewerbungen').insert({ student_user_id: userId, title, company, status, agent_name: agent })
  if (error) throw error
}
